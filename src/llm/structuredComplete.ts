...
function candidateInputs({ message }: { message: LlmMessage }): readonly unknown[] {
  ...
  return message.content
    ...
    .flatMap((block) => extractJsonObjects({ text: block.text ?? '' }))
}
...
export async function completeStructured<T>(
  { request, client, validate, maxAttempts = 2 }: {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      message = await client.complete({
        ...
        tools: [{ name: request.toolName, description: request.toolDescription, inputSchema: request.schema }],
