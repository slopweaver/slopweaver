...
export function buildPrompt({ params }: { params: LlmCreateParams }): string {
  ...
  if (tool !== undefined) {
    parts.push(`Respond with ONLY a JSON object that conforms to this JSON schema:\n${JSON.stringify(tool.inputSchema)}`)
  }
...
export function envelopeToMessage({ stdout }: { stdout: string }): LlmMessage {
  ...
  if (recovered !== undefined) {
    content.push({ type: 'tool_use', input: recovered })
  }
      ...
      return new Promise<LlmMessage>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdout.length > MAX_BUFFER) {
            kill()
            finish(() => { reject(new Error('claude CLI output exceeded buffer')) })
          }
        })
        child.on('error', (error) => { finish(() => { reject(error) }) })
        child.on('close', (code) => {
          if (code !== 0) {
            finish(() => { reject(new Error(`claude CLI exited with code ${String(code)}`)) })
            return
          }
          if (stdout.trim().length === 0) {
            finish(() => { reject(new Error('claude CLI produced no output')) })
            return
          }
          finish(() => {
            try {
