...
export function hybridSearch(
  { index, query, queryVector, vectorIndex, limit, alpha = DEFAULT_ALPHA, decay, candidatePool = CANDIDATE_POOL, bm25, relevanceFloor = 0 }: {
  ...
  const lexicalRaw = new Map(searchScored({ index, query, limit: candidatePool, ...(bm25 !== undefined ? { bm25 } : {}) }))
  const semanticRaw = queryVector !== undefined && vectorIndex !== undefined
    ? new Map(cosineTopN({ index: vectorIndex, queryVector, limit: candidatePool }))
