...
export type ExtractionPipeline =
  (texts: readonly string[], opts: { pooling: 'mean'; normalize: boolean }) => Promise<{ data: Iterable<number> }>
...
export async function embedInBatches(
  { texts, run, batchSize, prefix }: { texts: readonly string[]; run: ExtractionPipeline; batchSize: number; prefix: string },
  ...
  for (let i = 0; i < texts.length; i += batchSize) {
    ...
    vectors.push(...splitBatchVectors({ data: result.data, count: batch.length }))
  }
  return vectors
}
 ...
 */
export function loadPipeline({ importer = importTransformers }: { importer?: TransformersImporter } = {}): Promise<Result<ExtractionPipeline>> {
...
async function embedWith({ texts, prefix }: { texts: readonly string[]; prefix: string }): Promise<readonly Float32Array[]> {
  const pipeline = await loadPipeline()
  if (pipeline.ok === false) {
    throw new Error(pipeline.errors.join('; '))
  }
  return embedInBatches({ texts, run: pipeline.value, batchSize: resolveEmbedBatchSize({ raw: process.env.SLOPWEAVER_EMBED_BATCH }), prefix })
}
...
export const defaultEmbedder: Embedder = {
  ...
  embedDocuments: (texts) => embedWith({ texts, prefix: DOCUMENT_PREFIX }),
  embedQuery: (texts) => embedWith({ texts, prefix: QUERY_PREFIX }),
}
