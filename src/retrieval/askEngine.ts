...
export function retrieveRecords(
  { question, records, sliceLimit, decay, semantic, alpha }: {
    question: string
  ...
  const ids = semantic !== undefined
    ...
    : search({ index, query: question, limit: sliceLimit, ...(decay !== undefined ? { decay } : {}) })
...
export function answerQuestion(
  { question, client, records, sliceLimit, decay, semantic, alpha }: {
    question: string
    client: LlmClient
    records: readonly CorpusRecord[]
    sliceLimit: number
  ...
  return answerFromSlice({ question, client, slice })
}
