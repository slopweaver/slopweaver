...
export async function prepareSemanticContext(
  { records, query, deps, enabled, warn }: {
  ...
  const degrade = ({ reason }: { reason: string }): SemanticPreparation => {
    warn?.(`[slopweaver] semantic search requested but the embedder is unavailable; falling back to BM25-only (${reason})`)
  ...
  try {
    ...
    const vectorIndex = await buildVectorIndex({ records, embedder: deps.embedder, store: deps.store, persist: true })
