...
export function vectorContentHash({ modelId, record }: { modelId: string; record: CorpusRecord }): string {
  return createHash('sha256').update(`${modelId}\0${String(MAX_EMBED_CHARS)} ${recordText({ record })}`).digest('hex')
}
...
export async function buildVectorIndex(
  { records, embedder, store, persist = false }: {
  ...
  for (const record of records) {
    ...
    if (hit !== undefined && hit.contentHash === hash) {
  ...
  if (persist && (fresh.size > 0 || ids.length !== cached.size)) {
    await store.save(ids.map((id, i) => ({ sourceId: id, contentHash: hashById.get(id) ?? '', vector: vectors[i] ?? new Float32Array() })))
  }
  return { ids, vectors }
}
...
export function cosineTopN(
  { index, queryVector, limit }: { index: VectorIndex; queryVector: Float32Array; limit: number },
): readonly (readonly [string, number])[] {
  if (limit <= 0) {
    return []
  }
  const scored = index.ids.map((id, i): readonly [string, number] => [id, cosine({ a: queryVector, b: index.vectors[i] ?? new Float32Array() })])
...
export function inMemoryVectorCacheStore({ seed = [] }: { seed?: readonly CachedVector[] } = {}): VectorCacheStore {
  ...
  return {
    load: async () => stored,
    save: async (vectors) => { stored = vectors },
  }
}
