...
const USAGE = 'usage: slopweaver distil [--home <dir>] [--corpus <dir>] [--max-per-batch N] [--top-containers N] [--recent-only] [--dry-run]'

/** Reduce fresh+cached digests into per-source silver digests. */
function toSourceDigests({ digests, records }: { digests: readonly BatchDigest[]; records: readonly CorpusRecord[] }): readonly SourceDigest[] {
  ...
  return [...bySource.entries()].map(([source, sourceDigests]) =>
    reduceToSourceDigest({ source, digests: sourceDigests, recordCount: records.filter((r) => r.source === source).length }))
}
...
export async function runDistil(argv: readonly string[]): Promise<number> {
  ...
  const parsed = parseFlagTail({
    rest,
    spec: { value: ['home', 'corpus', 'max-per-batch', 'top-containers'], boolean: ['recent-only', 'dry-run'] },
  })
  if (parsed.ok === false) {
  ...
  const maxPerBatch = values['max-per-batch'] !== undefined
    ? parsePositiveInteger({ value: values['max-per-batch'], label: '--max-per-batch', errors: flagErrors })
    : undefined
  const topContainers = values['top-containers'] !== undefined
    ? parsePositiveInteger({ value: values['top-containers'], label: '--top-containers', errors: flagErrors })
    : undefined
  ...
  if (dir.ok === false) {
    dir.errors.forEach((e) => { logger.error(e) })
    return EXIT_ERROR
  }
  const read = readCorpusDir({ dir: dir.value })
  read.warnings.forEach((w) => { logger.warn(w) })
  ...
  if (flags.has('dry-run')) {
    ...
    logger.out(`${String(batches.length)} batch(es): ${String(hits)} cached, ${String(batches.length - hits)} would call the model`)
    return EXIT_OK
  }
  if (batches.length === 0) {
  ...
  run.errors.forEach((e) => { logger.warn(`skip ${e}`) })
  ...
  logger.out(`distilled ${String(run.called)} batch(es) (LLM) + ${String(run.hits)} cached → gold at ${goldDir({ home })}`)
  return EXIT_OK
}
