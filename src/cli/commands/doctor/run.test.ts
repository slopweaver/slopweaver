...
let home: string
beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'slop-doctor-')) })
afterEach(() => { rmSync(home, { recursive: true, force: true }) })

describe('doctorReport', () => {
  it('reports an initialised home: version, corpus roots, valid seeds', () => {
    runInit({ home })
    const lines = doctorReport({ home, envHome: home, version: '9.9.9' })
  ...
  it('reports an uninitialised home as not-initialised, pointing at init', () => {
    const lines = doctorReport({ home, envHome: undefined, version: '9.9.9' })
  ...
  it('is read-only — reporting creates nothing', () => {
    doctorReport({ home, envHome: home, version: '9.9.9' })
