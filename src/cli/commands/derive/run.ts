...
export function runDerive(argv: readonly string[]): number {
  ...
  const parsed = parseFlagTail({ rest, spec: { value: ['home', 'corpus', 'top'], boolean: ['dry-run'] } })
  if (parsed.ok === false) {
  ...
  const top = values.top !== undefined ? parsePositiveInteger({ value: values.top, label: '--top', errors: flagErrors }) : DEFAULT_TOP
  ...
  const artifacts = deriveSilver({ records: read.ok ? read.value : [], identityMap: loadIdentityMap({ home }) })
  ...
  planDeriveSummary({ artifacts, top }).forEach((line) => { logger.out(line) })
