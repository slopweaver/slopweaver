...
function overviewDoc({ people, containers, sources, opportunities, builtAtIso }: GoldInput): GoldDoc {
  ...
  const lines = [
    ...
    '## By source',
    '',
    ...sources.map((s) => `- **${s.source}** — ${String(s.recordCount)} records, ${String(s.containers.length)} containers`),
    '',
    '## Busiest containers',
    '',
    ...
    '',
    ...crossCutting({ opportunities }).slice(0, 12).map((o) => `- ${o.summary} _(score ${String(o.score)})_`),
    '',
  ]
  return { path: 'overview.md', markdown: lines.join('\n') }
}
...
function bySourceDoc({ digest }: { digest: SourceDigest }): GoldDoc {
  ...
  return { path: `by-source/${digest.source}.md`, markdown: lines.join('\n') }
}
...
function whereToLookDoc({ people, sources, opportunities }: GoldInput): GoldDoc {
  ...
  lines.push('')
  return { path: 'where-to-look.md', markdown: lines.join('\n') }
}
...
export function buildGoldDocs(input: GoldInput): readonly GoldDoc[] {
  return [
    overviewDoc(input),
    ...input.sources.map((digest) => bySourceDoc({ digest })),
    whereToLookDoc(input),
  ]
}
