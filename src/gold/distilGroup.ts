...
function hashBatch({ source, container, records }: { source: CorpusSource; container: string; records: readonly CorpusRecord[] }): string {
...
function byRecency({ a, b }: { a: CorpusRecord; b: CorpusRecord }): number {
  return a.tsIso < b.tsIso ? 1 : a.tsIso > b.tsIso ? -1 : a.sourceId < b.sourceId ? -1 : a.sourceId > b.sourceId ? 1 : 0
}

/** Keep only the busiest `n` containers per source (drops the low-signal long tail). */
function keepTopContainers(
  { buckets, n }: { buckets: Map<string, CorpusRecord[]>; n: number },
  ...
  for (const [key, records] of buckets) {
    const source = (records[0]?.source ?? 'github') as CorpusSource
...
export function groupForDistil(
  { records, maxPerBatch = DEFAULT_MAX_PER_BATCH, topContainersPerSource, recentOnly = false }: {
  ...
  const scoped = topContainersPerSource !== undefined ? keepTopContainers({ buckets, n: topContainersPerSource }) : buckets

  const batches: DistilBatch[] = []
  for (const key of [...scoped.keys()].sort()) {
    const bucket = [...(scoped.get(key) ?? [])].sort((a, b) => byRecency({ a, b }))
    const source = (bucket[0]?.source ?? 'github') as CorpusSource
    const container = bucket[0]?.container ?? ''
    const slices = recentOnly ? [bucket.slice(0, maxPerBatch)] : chunk({ items: bucket, size: maxPerBatch })
    for (const slice of slices) {
      if (slice.length > 0) {
        batches.push({ source, container, records: slice, hash: hashBatch({ source, container, records: slice }) })
      }
    }
  }
  return batches
