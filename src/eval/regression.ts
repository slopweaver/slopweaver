...
import { GOLDEN_CASES, scoreGrounding, type GoldenCase, type QuestionClass } from './scorer.js'
...
function caseRecall(
  { golden, index, nowMs, k, halfLifeDays }: {
    golden: GoldenCase
  ...
  const ids = search({ index, query: golden.question, limit: k, decay: decayParamsFromDays({ days: halfLifeDays, nowMs }) })
  const score = scoreGrounding({
    expectedGrounding: golden.expectedGrounding,
    answer: { retrievedRefs: ids.map((id) => ({ sourceId: id, token: id })), citedTokens: [] },
  })
  return { question: golden.question, kind: golden.kind, recall: score.retrievalRecall, hits: score.retrievedHits, expected: score.expectedCount }
}
...
export function scoreRecall(
  { records, cases = GOLDEN_CASES, nowMs, k = RECALL_K, halfLifeDays = RECALL_HALF_LIFE_DAYS }: {
  ...
  const rows = cases.map((golden) => caseRecall({ golden, index, nowMs, k, halfLifeDays }))
  ...
  return { overall: mean({ values: rows.map((r) => r.recall) }), clusters, cases: rows }
}
...
export function compareToBaseline(
  { candidate, baseline }: { candidate: RecallScore; baseline: RecallBaseline },
): RegressionDiff {
  const failures: RegressionFailure[] = []
  if (candidate.overall < baseline.overallFloor - FLOOR_EPSILON) {
    failures.push({ scope: 'overall', floor: baseline.overallFloor, actual: candidate.overall, delta: candidate.overall - baseline.overallFloor })
  }
  for (const [scope, floor] of Object.entries(baseline.clusterFloors)) {
    ...
    const actual = candidate.clusters[scope as QuestionClass] ?? 0
    if (actual < floor - FLOOR_EPSILON) {
      failures.push({ scope, floor, actual, delta: actual - floor })
    }
  }
  return {
    ok: failures.length === 0,
    baseline: { overall: baseline.overallFloor, clusters: baseline.clusterFloors },
    candidate: { overall: candidate.overall, clusters: candidate.clusters },
    failures,
  }
}
...
export function buildBaseline(
  { score, reason, nowIso = REGRESSION_NOW_ISO }: { score: RecallScore; reason: string; nowIso?: string },
): RecallBaseline {
  return {
    schemaVersion: 1,
