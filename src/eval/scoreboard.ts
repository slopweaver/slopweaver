...
function rangeStat({ values }: { values: readonly number[] }): RangeStat {
  return { max: Math.max(...values), median: median({ values }), min: Math.min(...values) };
}
...
export function renderScoreboard({ rows }: { rows: readonly CaseAggregate[] }): string {
  ...
  const lines: string[] = [];
  lines.push(
    `**Mean retrieval recall@k: ${pct({ ratio: meanRetrieval })}** across ${String(rows.length)} cases` +
  ...
  for (const row of rows) {
    ...
    const retrieval =
      `${pct({ ratio: row.retrievalRecall })} (${String(Math.round(row.retrievalRecall * row.expectedCount))}/${String(row.expectedCount)})` +
      (row.retrievalStable ? "" : " ⚠️unstable");
    lines.push(
      `| ${flag} | ${row.kind} | ${row.question} | ${retrieval} | ${pctRange({ stat: row.answerRecall })} | ${pctRange({ stat: row.citationPrecision })} |`,
    );
  }
  return lines.join("\n");
}
