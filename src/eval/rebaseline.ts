...
export function decideRebaseline(
  { args, ci, allowInCi }: { args: readonly string[]; ci: boolean; allowInCi: boolean },
  ...
  if (ci && !allowInCi) {
    return err(['refusing to re-baseline in CI. Re-baseline locally and commit, or set SLOPWEAVER_ALLOW_REBASELINE_IN_CI=1 to override.'])
  }
...
export function runRebaseline(argv: readonly string[]): number {
  ...
  if (decision.ok === false) {
    decision.errors.forEach((e) => { process.stderr.write(`rebaseline: ${e}\n`) })
    return 1
  }
  const records = loadFixtureRecords({ path: fixturePath() })
  const score = scoreRecall({ records, nowMs: Date.parse(REGRESSION_NOW_ISO) })
  const baseline = buildBaseline({ score, reason: decision.value.reason })
  writeFileSync(baselinePath(), `${JSON.stringify(baseline, null, 2)}\n`, 'utf8')
  process.stderr.write(`rebaseline: wrote ${baselinePath()} — overall recall@${String(baseline.k)} floor ${(baseline.overallFloor * 100).toFixed(1)}% (reason: ${baseline.reason})\n`)
  return 0
}
