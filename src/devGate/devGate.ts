...
export function prFormatResult({ body }: { body: string | undefined }): GateCheckResult {
  if (body === undefined || body.trim().length === 0) {
    return {
      name: "pr-format",
      ok: false,
      summary: "no PR body provided (pass --pr-body-file <path> or set $PR_BODY)",
    };
  }
...
export function evalRegressionResult({
  ...
  const candidate = scoreRecall({
    halfLifeDays: baseline.halfLifeDays,
    k: baseline.k,
    nowMs: Date.parse(baseline.nowIso),
    records,
  });
  const diff = compareToBaseline({ baseline, candidate });
  ...
  return { diff, result: { name: "eval-regression", ok: diff.ok, summary } };
}
 ...
 */
export function composeGate({ results }: { results: readonly GateCheckResult[] }): {
  readonly ok: boolean;
...
export function runDevGate(argv: readonly string[]): number {
  ...
  const parsed = parseFlags({
    ...
    args: argv.slice(3),
    spec: { string: ["home", "pr-body-file"] },
  });
  if (parsed.ok === false) {
    parsed.errors.forEach((e) => {
      process.stderr.write(`dev gate: ${e}\n`);
    });
    process.stderr.write("usage: slopweaver dev gate [--home <dir>] [--pr-body-file <path>]\n");
    return EXIT_USAGE;
  }
  const { values } = parsed.value;
  const home = flagValue({ key: "home", values });
  ...
  const hygiene: GateCheckResult = {
    name: "hygiene",
    ok: hygieneCode === 0,
    summary: hygieneCode === 0 ? "clean" : "leak-class/denylist hit(s) — see above",
  };

  // 2) eval-regression — deterministic recall over the frozen fixture vs the frozen baseline.
  const { result: evalResult, diff } = evalRegressionResult({
    baseline: loadBaseline(),
    records: loadFixtureRecords({ path: fixturePath() }),
  });

  // 3) pr-format — the description schema.
  const prFormat = prFormatResult({ body: readPrBody({ bodyFile: flagValue({ key: "pr-body-file", values }) }) });
  ...
  writeFileSync(join(paths.ledgers, "dev-gate.jsonl"), `${gateLogLines({ results, runId, tsIso }).join("\n")}\n`, {
    flag: "a",
  });
