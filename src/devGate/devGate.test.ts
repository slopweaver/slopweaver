...
describe("evalRegressionResult", () => {
  it("passes the full frozen fixture against its own baseline", () => {
    const { result, diff } = evalRegressionResult({
  ...
  it("fails when the corpus is degraded below the baseline floors", () => {
    ...
    const { result, diff } = evalRegressionResult({ baseline: loadBaseline(), records: degraded });
    expect(result.ok).toBe(false);
...
describe("composeGate", () => {
  ...
  it("passes (exit 0) only when every check passes", () => {
    expect(composeGate({ results: [pass, { name: "eval-regression", ok: true, summary: "ok" }] })).toEqual({
      exit: 0,
      ok: true,
    });
  });
  ...
  it("fails (exit 1) when any check fails", () => {
    expect(composeGate({ results: [pass, fail] })).toEqual({ exit: 1, ok: false });
  });
});
...
describe("gateLogLines", () => {
  it("emits one JSONL line per check with the run id, status, and summary", () => {
    ...
    const lines = gateLogLines({ results, runId: "run-1", tsIso: "2026-07-14T00:00:00.000Z" });
    expect(lines).toHaveLength(2);
    expect(parseLogLine({ line: lines[0]! })).toEqual({
      check: "hygiene",
      runId: "run-1",
      schemaVersion: 1,
      status: "pass",
    ...
    expect(parseLogLine({ line: lines[1]! })).toEqual({
      check: "pr-format",
      runId: "run-1",
      schemaVersion: 1,
      status: "fail",
