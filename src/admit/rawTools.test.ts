...
import { classifyRawCommand, RAW_ESCAPE } from "./rawTools.js";
...
function isBlocked({ command }: { command: string }): boolean {
  return classifyRawCommand({ allowRaw: false, command }).blocked;
}
...
function blockedMessage({ command }: { command: string }): string {
  const verdict = classifyRawCommand({ allowRaw: false, command });
  if (!verdict.blocked) {
...
describe("classifyRawCommand", () => {
  ...
  it("allows a would-be-blocked command under the SLOPWEAVER_ALLOW_RAW escape", () => {
    const verdict = classifyRawCommand({ allowRaw: true, command: "gh pr merge 1" });
