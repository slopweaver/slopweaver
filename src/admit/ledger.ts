...
export function doorLedgerLine(
  { request, decision, runId, tsIso }: { request: DoorRequest; decision: DoorDecision; runId: string; tsIso: string },
): string {
...
export function recordDoorDecision(
  { request, decision, runId, tsIso, home }: {
    request: DoorRequest
  ...
  appendFileSync(join(ledgers, 'door.jsonl'), `${doorLedgerLine({ request, decision, runId, tsIso })}\n`, 'utf8')
}
