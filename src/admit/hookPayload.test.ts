...
describe('evaluateHookPayload', () => {
  it('blocks a raw mutating Bash command with the escape message', () => {
    const decision = evaluateHookPayload({ payload: bash({ command: 'gh pr merge 1' }), allowRaw: false })
    expect(decision.block).toBe(true)
    expect(blockedMessage({ decision })).toContain(RAW_ESCAPE)
  })

  it('allows a read-only Bash command', () => {
    expect(evaluateHookPayload({ payload: bash({ command: 'git status' }), allowRaw: false })).toEqual({ block: false })
  })

  it('allows a would-be-blocked command under the escape', () => {
    expect(evaluateHookPayload({ payload: bash({ command: 'gh pr merge 1' }), allowRaw: true })).toEqual({ block: false })
  })

  it('allows a well-formed non-Bash tool', () => {
    expect(evaluateHookPayload({ payload: { tool_name: 'Write', tool_input: { file_path: '/x', content: 'y' } }, allowRaw: false })).toEqual({ block: false })
  })

  it('BLOCKS loudly on a malformed (non-object) payload — fail closed, not fail open', () => {
    const decision = evaluateHookPayload({ payload: 'not-an-object', allowRaw: false })
    expect(decision.block).toBe(true)
    expect(blockedMessage({ decision })).toContain('malformed')
  })

  it('BLOCKS loudly on a Bash payload missing its command', () => {
    const decision = evaluateHookPayload({ payload: { tool_name: 'Bash', tool_input: {} }, allowRaw: false })
    expect(decision.block).toBe(true)
