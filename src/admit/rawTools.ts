...
const GIT_VALUE_GLOBALS = new Set(['-C', '-c', '--git-dir', '--work-tree', '--namespace', '--exec-path', '--super-prefix'])
...
function gitSubcommand({ tokens }: { tokens: readonly string[] }): { sub: string | undefined; flags: readonly string[] } {
  let i = 1
  while (i < tokens.length) {
    const token = tokens[i]!
    if (token.startsWith('-')) {
    ...
    return { sub: token, flags: tokens.slice(i + 1) }
  }
  return { sub: undefined, flags: [] }
}

/** The first two non-flag tokens after `gh` — its group + subcommand. */
function ghGroupSub({ tokens }: { tokens: readonly string[] }): { group: string | undefined; sub: string | undefined; rest: readonly string[] } {
  const words = tokens.slice(1).filter((t) => !t.startsWith('-'))
  return { group: words[0], sub: words[1], rest: tokens.slice(1) }
}
...
function isMutatingGh({ tokens }: { tokens: readonly string[] }): boolean {
  ...
  if (group === 'api') {
    const hasFields = rest.some((t) => t === '-f' || t === '-F' || t === '--field' || t === '--raw-field' || t === '--input')
...
export function classifyRawCommand({ command, allowRaw }: { command: string; allowRaw: boolean }): RawVerdict {
  const tokens = stripEnvPrefix({ tokens: command.trim().split(/\s+/).filter((t) => t.length > 0) })
