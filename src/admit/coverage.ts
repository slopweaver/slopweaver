...
const SEAM_PATTERNS: readonly { readonly seam: string; readonly re: RegExp }[] = [
  { seam: 'writeFileSync', re: /\bwriteFileSync\s*\(/ },
  { seam: 'appendFileSync', re: /\bappendFileSync\s*\(/ },
  { seam: 'mkdirSync', re: /\bmkdirSync\s*\(/ },
  { seam: 'rmSync', re: /\brmSync\s*\(/ },
  { seam: 'unlinkSync', re: /\bunlinkSync\s*\(/ },
  { seam: 'renameSync', re: /\brenameSync\s*\(/ },
  { seam: 'spawn', re: /\bspawn(?:Sync)?\s*\(/ },
  { seam: 'execFile', re: /\bexecFile(?:Sync)?\s*\(/ },
  { seam: 'exec', re: /\bexecSync\s*\(/ },
]
  ...
  for (const noun of Object.keys(groups)) {
    ...
    for (const verb of Object.keys(verbs)) {
      ...
      if (effect === 'external-write' && doorRouted !== true) {
        gaps.push({ noun, verb, reason: 'external-write-not-routed' })
      }
    }
  }
  return gaps
 ...
 */
export function analyzeCoverage({ files, groups }: { files: readonly SourceFile[]; groups: NounGroups }): CoverageReport {
  const seams: SeamHit[] = []
  ...
  return { seams, open, verbGaps: gaps, ok: open.length === 0 && gaps.length === 0 }
}
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (entry.name !== 'node_modules') {
        out.push(...collectSources({ root, dir: full }))
      }
...
export function coverageReport(): CoverageReport {
  const root = repoRoot()
  const files = [
    ...collectSources({ root, dir: join(root, 'src') }),
    ...collectSources({ root, dir: join(root, 'eval') }),
    ...collectSources({ root, dir: join(root, 'hooks') }),
  ]
