...
function spotCrossCutting(
  { records, nowMs, viaTokens }: { records: readonly CorpusRecord[]; nowMs: number; viaTokens: ReadonlySet<string> },
): readonly Opportunity[] {
  const byToken = new Map<string, CrossCutAccum>()
  for (const record of records) {
    const recency = recencyWeight({ tsIso: record.tsIso, nowMs })
...
function spotBlockers({ records, nowMs }: { records: readonly CorpusRecord[]; nowMs: number }): readonly Opportunity[] {
  ...
  for (const record of records) {
    ...
    const stale = recencyWeight({ tsIso: record.tsIso, nowMs }) <= 0
...
function spotDuplication({ records, nowMs }: { records: readonly CorpusRecord[]; nowMs: number }): readonly Opportunity[] {
  ...
  for (const record of records) {
    ...
    const accum = byTitle.get(key) ?? { raw: record.title, sources: new Set<CorpusSource>(), evidence: new Set<string>(), recency: 0 }
    accum.sources.add(record.source)
    accum.evidence.add(evidenceOf({ record }))
    accum.recency = Math.max(accum.recency, recencyWeight({ tsIso: record.tsIso, nowMs }))
...
export function spotOpportunities(
  { records, edges }: { records: readonly CorpusRecord[]; edges: readonly GraphEdge[] },
): readonly Opportunity[] {
  const nowMs = corpusNowMs({ records })
  const viaTokens = new Set(edges.map((edge) => edge.via))
  const all = [
    ...spotCrossCutting({ records, nowMs, viaTokens }),
    ...spotBlockers({ records, nowMs }),
    ...spotDuplication({ records, nowMs }),
  ]
  return all.sort((a, b) =>
    b.score - a.score
